 ...
 */
/************************************************************************************************/
void KsForwardPlusRender::createRenderState()
{
	KsRenderSystem* pRenderSystem = m_pRenderSystem;

	// ブレンドステート
	{
		KS_BLEND_DESC			desc = KsBlendState::DEFAULT;

        desc.AlphaToCoverageEnable                 = ksFALSE;
        desc.IndependentBlendEnable                = ksFALSE;
        desc.RenderTarget[0].BlendEnable           = ksTRUE;
        desc.RenderTarget[0].BlendOp               = KS_BLEND_OP_ADD;
        desc.RenderTarget[0].SrcBlend              = KS_BLEND_ONE; 
        desc.RenderTarget[0].DestBlend             = KS_BLEND_ONE; 
        desc.RenderTarget[0].BlendOpAlpha          = KS_BLEND_OP_ADD;
        desc.RenderTarget[0].SrcBlendAlpha         = KS_BLEND_ZERO; 
        desc.RenderTarget[0].DestBlendAlpha        = KS_BLEND_ONE; 
        desc.RenderTarget[0].RenderTargetWriteMask = KS_COLOR_WRITE_ENABLE_ALL;

		m_pAdditiveBlendingState = pRenderSystem->createBlendState( &desc );
	}

	// 深度ステンシルステート
	{
		KS_DEPTH_STENCIL_DESC	desc = KsDepthStencilState::DEFAULT;
		desc.DepthEnable      = ksTRUE;
		desc.DepthWriteMask   = KS_DEPTH_WRITE_MASK_ZERO;
		desc.DepthFunc        = KS_COMPARISON_GREATER;
		desc.StencilEnable    = ksFALSE;
		desc.StencilReadMask  = KS_DEFAULT_STENCIL_READ_MASK;
		desc.StencilWriteMask = KS_DEFAULT_STENCIL_WRITE_MASK;

		m_pDisableDepthWrite  = pRenderSystem->createDepthStencilState( &desc );

		desc.DepthEnable      = ksFALSE;
		m_pDisableDepthTest   = pRenderSystem->createDepthStencilState( &desc );
	}

	// ラスタライザーステート
	{
		KS_RASTERIZER_DESC		desc = KsRasterizerState::DEFAULT;
        desc.FillMode              = KS_FILL_SOLID;
        desc.CullMode              = KS_CULL_NONE;  // disable culling
        desc.FrontCounterClockwise = ksFALSE;
        desc.DepthBias             = 0;
        desc.DepthBiasClamp        = 0.0f;
        desc.SlopeScaledDepthBias  = 0.0f;
        desc.DepthClipEnable       = ksTRUE;
        desc.ScissorEnable         = ksFALSE;
        desc.MultisampleEnable     = ksFALSE;
        desc.AntialiasedLineEnable = ksFALSE;
		m_pDisableCulling          = pRenderSystem->createRasterizerState( &desc );
	}
}
 ...
 */
/************************************************************************************************/
KsForwardPlusRender::KsForwardPlusRender( KsRenderSystem* pRenderSystem, KsUInt32 width, KsUInt32 height, KsUInt32 MSAA )
	: m_pRenderSystem( pRenderSystem )
	, m_pRenderContext( 0 )

	, m_pAdditiveBlendingState( 0 )
	, m_pDisableDepthWrite( 0 )
	, m_pDisableDepthTest( 0 )
	, m_pDisableCulling( 0 )
	...
	*/
{
	...
	// ブレンドステート
	{
		...
		// 不透明ブレンド設定
		m_pOpaqueState = pRenderSystem->createBlendState( &desc );

		// アルファブレンドステート
		desc.AlphaToCoverageEnable = ksTRUE;
		m_pAlphaToCoverageState    = pRenderSystem->createBlendState( &desc );
	}

	// ラスタライズステート
	{
		KS_DEPTH_STENCIL_DESC	desc = KsDepthStencilState::DEFAULT;
		m_pDepthGreater = pRenderSystem->createDepthStencilState( &desc );

		desc.DepthWriteMask = KS_DEPTH_WRITE_MASK_ZERO; 
		desc.DepthFunc      = KS_COMPARISON_EQUAL; 
		m_pDepthEqualAndDisableDepthWrite = pRenderSystem->createDepthStencilState( &desc );
	}
	...
	KsCameraManager::getInstance().addCamera( m_pCamera );
	KsCameraManager::getInstance().setActiveCamera( m_pCamera );
#endif
}
 ...
 */
/************************************************************************************************/
KsForwardPlusRender::~KsForwardPlusRender()
{
	ksDELETE( m_pAdditiveBlendingState );
	ksDELETE( m_pDisableDepthWrite );
	ksDELETE( m_pDisableDepthTest );
	ksDELETE( m_pDisableCulling );
}
 ...
 */
/************************************************************************************************/
void KsForwardPlusRender::setViewport( KsInt32 x, KsInt32 y, KsInt32 w, KsInt32 h, KsReal minZ, KsReal maxZ, KsRenderContext* pRenderContext )
{
	// ビューポート設定
 ...
 */
/************************************************************************************************/
void KsForwardPlusRender::setRenderTarget_MRT( KsRenderContext* pRenderContext )
{
	// MRTセット
 ...
 */
/************************************************************************************************/
void KsForwardPlusRender::setRenderTarget_Default( KsRenderContext* pRenderContext )
{
	// レンダーターゲットを元に戻す
 ...
 */
/************************************************************************************************/
void KsForwardPlusRender::clearScene()
{
	// ターゲットをクリアーする
	if( m_isMRT )
	{
 ...
 */
/************************************************************************************************/
void KsForwardPlusRender::clearColor()
{
	// ターゲットをクリアーする
	if( m_isMRT )
	{
 ...
 */
/************************************************************************************************/
void KsForwardPlusRender::clearDepthStencil()
{
	// 深度ステンシルバッファをクリアーする
 ...
 */
/************************************************************************************************/
void KsForwardPlusRender::initializeLight()
{
}
 ...
 */
/************************************************************************************************/
void KsForwardPlusRender::updateShaderParameter()
{
	//KsReal			fNear       = 0.1f;
	//KsReal			fFar        = 1000.0f;
 ...
 */
/************************************************************************************************/
void KsForwardPlusRender::updateLight()
{
 ...
 */
/************************************************************************************************/
void KsForwardPlusRender::renderGBuffer()
{
	// 深度ステンシルバッファをクリアーする
 ...
 */
/************************************************************************************************/
void KsForwardPlusRender::computeLighting()
{
 ...
 */
/************************************************************************************************/
void KsForwardPlusRender::finalRender()
{
 ...
 */
/************************************************************************************************/
void KsForwardPlusRender::renderContourExtraction()
{
 ...
 */
/************************************************************************************************/
void KsForwardPlusRender::drawDebug()
{
 ...
 */
/************************************************************************************************/
void KsForwardPlusRender::render()
{
	// 別スレッドの描画スレッド処理
 ...
 */
/************************************************************************************************/
void KsForwardPlusRender::addModel( KsModel* pModel )
{
 ...
 */
/************************************************************************************************/
void KsForwardPlusRender::addActor( KsActor* pActor )
{
