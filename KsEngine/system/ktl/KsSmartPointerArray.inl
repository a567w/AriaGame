...
/************************************************************************************************/
template< typename T > 
KsSmartPtrArray< T >::KsSmartPtrArray()
	: m_ptr( NULL )
	, m_num( 0 )
	, m_sizeof( 0 )
{
...
template< typename T >
KsSmartPtrArray< T >::KsSmartPtrArray( T* p, int num = 0 )
	: m_ptr( p )
	, m_num( num )
	, m_sizeof( m_sizeof(T) )
{
 ...
 */
template< typename T >
KsSmartPtrArray< T >::KsSmartPtrArray( const KsSmartPtrArray<T>& rhs )
	: m_ptr( rhs.m_ptr )
	, m_num( rhs.m_num )
	, m_sizeof( rhs.m_sizeof )
{
	...
	//m_pPrev = const_cast<KsSmartPtrArray<T>*>( &rhs );
	m_pPrev = &rhs;
	m_pPrev->m_pNext = this;
}
...
*/
template< typename T >
void KsSmartPtrArray< T >::destroy()
{
	if( m_pNext == this ){
		delete m_ptr;
	}
...
*/
template< typename T >
KsSmartPtrArray<T>&	KsSmartPtrArray< T >::operator=( const KsSmartPtrArray<T>& rhs )
{
	if( m_ptr != rhs.m_ptr ){
...
*/
template< typename T >
KsSmartPtrArray<T>&	KsSmartPtrArray< T >::operator=( const T* ptr )
{
	//とりあえず開放処理
	destroy();
